...
-- Data per schema per catalog
SELECT `table_catalog`, `table_schema`, `ts`, SUM(`data_length`), SUM(`index_length`), SUM(`data_free`)
  FROM `table_size`
 WHERE `table_catalog` = 'def'
   AND `table_schema` = 'dbstat'
...
-- Data per catalog
SELECT `table_catalog`, `ts`, SUM(`data_length`), SUM(`index_length`), SUM(`data_free`)
  FROM `table_size`
 WHERE `table_catalog` = 'def'
...
SELECT `engine`, `ts`, SUM(`data_length`), SUM(`index_length`), SUM(`data_free`)
  FROM `table_size`
 GROUP BY `engine`, `ts`
